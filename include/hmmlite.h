...
class Labfile /*{{{*/
{
	public:
		...
		const char *getFname(){return s_fname.c_str();}
		int getNumLab(){return num_lab;}
		int getStartF(int t){return start_f[t];};
		int getEndF(int t){return end_f[t];};
		int getDuration(int t){return end_f[t] - start_f[t] + 1;}
		int getDuration_1(int t){return end_f[t] - start_f[t];}
		int getCluster(int t){return cluster[t];}
